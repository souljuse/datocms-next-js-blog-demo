...
- `NEXT_DATOCMS_API_TOKEN_PREVIEW` can be any random string (but avoid spaces), like `MY_SECRET` - this is used for [the Preview Mode](/docs/advanced-features/preview-mode.md).

Your `.env` file should look like this:

```bash
NEXT_DATOCMS_API_TOKEN=...
NEXT_DATOCMS_API_TOKEN_PREVIEW=...
```
...
- `<secret>` should be the string you entered for `NEXT_DATOCMS_API_TOKEN_PREVIEW`.
